...
### Read data
The data is read from the downloaded csv.vz2 file.
...
The subsetting and event type reduction is performed in the following section.
...
### Enrichment
The crop and property damages are each reported in two separate columns. E.g. the crop damage is reported in columns `CROPDMG` and `CROPDMGEXP`. One columns contains the number value, the other contains the exponent. In order to easiser perform comparisons two new columns containing the converted values is added.

```{r enrichment, echo=TRUE}

# Calculating the property and crop damage from the CROPDMG, CROPDMGEXP, PROPDMG and PROPDMGEXP columns. The calculation is based on mappings found on page 12 in the PDF referenced together with the data base.
# 0 -> 1
# K -> 1000
# M -> 1000000
# B -> 1000000000
...
caption <- tableGrob(matrix(expression(paste("Figure 1: Population health"))),
                     parse = TRUE, theme = 
                       theme.list(
...
caption <- tableGrob(matrix(expression(paste("Figure 2: Economic consequences"))),
                     parse = TRUE, theme = 
                       theme.list(
                         gpar.corefill = gpar(fill = NA, col = NA),
                         core.just = "center"
