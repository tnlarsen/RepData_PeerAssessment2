...
The result of the above cleanup is that the data now contains **`r dim(storm.data.filtered)[1]`** events spread over **`r length(unique(storm.data.filtered$EVTYPE))`** unique event types. The `r countToShow` most recorded event types are listed in the below table: 
...
caption <- tableGrob(matrix(expression(
    paste("Figure 1: Event types with highest population health consequence since 1996"))),
    parse = TRUE, theme = 
        theme.list(
...
caption <- tableGrob(matrix(expression(
    paste("Figure 2: Event types with highest economic consequence since 1996"))),
    parse = TRUE, theme = 
        theme.list(
            gpar.corefill = gpar(fill = NA, col = NA),
            core.just = "center"
